$(document).ready(function(){
	...
	function getAllLawyer() {
	 ...
	 $(document).on('click','#remove_lawyer',function(){
		 $.ajax({
			url: "../../controllers/userController.php",
			type: "POST",
			data: {type: "2", id: $(this).attr("class"),user_type: "lawyer"},
			success: function(result){
				alert("Delete");
				getAllLawyer();
		 }});
	...
	$(document).on('click','.update_lawyer_btn',function(){
		...
		$("#lawyer_name").attr("value", users_info[1]);
	 ...
	 $(document).on('click','.update_data',function(){	
		 $.ajax({
			...
			url: "../../controllers/userController.php",
			type: "POST",
			data: {type: "3", id: user_id, username: $("#lawyer_name").val(), nid:$("#nid").val(), phone:$("#phone").val(),user_type: "lawyer"},
			success: function(result){
